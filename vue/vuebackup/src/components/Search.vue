<template>
  <div id="content" class="row align-center font-override">
    <div class="col col-6">
      ...
      <div id="no-results" class="margin" v-if="!showResults && !searching && loggedIn">
        <div> {{ resultsBoxMessage }} </div>
      </div>
      <transition name="fade">
        <ul id="list" v-if="showResults">
  ...
  export default {
    ...
    data () {
      return {
        ...
        showResults: false,
        searching: false,
        resultsBoxMessage: 'Results will appear here when you search'
      }
    },
    computed: {
      loggedIn: function () {
        ...
        this.$http.get('/search/' + encodeURIComponent(this.artist.name), {
          before: () => {
            ...
            if (response.data) {
              ...
              this.resultsBoxMessage = 'No results for "' + this.artist.name + '"'
